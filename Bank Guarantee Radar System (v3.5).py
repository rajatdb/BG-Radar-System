...
class ExecutiveRailwaysPortal(tk.Tk):
    def __init__(self):
        ...
        # Security User Tag & Live Active Counter Badge in Header
        user_badge_frame = tk.Frame(header, bg="#0f2942", highlightbackground=self.AMBER, highlightthickness=1)
        user_badge_frame.pack(side="right", padx=20, pady=12)

        # Active User Counter Badge Button
        self.btn_active_count = tk.Button(
            user_badge_frame, text="🟢 Live Active Staff: 1",
            font=("Segoe UI", 10, "bold"), fg="#4ade80", bg="#0f2942",
            activebackground="#1e3a8a", activeforeground="#ffffff",
            relief="flat", bd=0, cursor="hand2", padx=8, pady=4,
            command=self.show_active_users_popup
        )
        self.btn_active_count.pack()

        lbl_user_icon = tk.Label(
            user_badge_frame, text=f"👤 Logged: {self.active_user}",
            font=("Segoe UI", 8), fg="#94a3b8", bg="#0f2942"
        )
        lbl_user_icon.pack(pady=(0, 2))
        ...
        self.start_active_user_counter_loop()
    ...
    # =========================================================================
    # HERE IS THE NEW METHOD TO ADD:
    # =========================================================================
    def start_active_user_counter_loop(self):
        """Periodically fetches active user count and updates top-right badge"""
        def update_badge_count():
            while True:
                if WEB_APP_URL and "YOUR_DEPLOYMENT_ID" not in WEB_APP_URL:
                    try:
                        req = urllib.request.Request(WEB_APP_URL, headers={'User-Agent': 'Mozilla/5.0'})
                        with urllib.request.urlopen(req, timeout=5) as resp:
                            users = json.loads(resp.read().decode('utf-8'))
                            online_users = [u for u in users if u.get('status') == 'ONLINE']
                            count = len(online_users)
                            
                            display_count = count if count > 0 else 1
                            
                            self.after(0, lambda c=display_count: self.btn_active_count.config(
                                text=f"🟢 Live Active Staff: {c}"
                            ))
                    except Exception:
                        pass
                time.sleep(30) # Refresh every 30 seconds

        threading.Thread(target=update_badge_count, daemon=True).start()

    def show_active_users_popup(self):
        """Step 1: Shows color-coded list (Green=ONLINE, Red=OFFLINE) when badge is clicked"""
        if not WEB_APP_URL or "YOUR_DEPLOYMENT_ID" in WEB_APP_URL:
            messagebox.showwarning("Configuration Needed", "Please set deployed Google Web App URL.")
            return

        # Create sleek Popup Window below button
        popup = tk.Toplevel(self)
        popup.title("Online Staff")
        popup.geometry("300x260")
        popup.configure(bg=self.NAVY)
        popup.transient(self)
        popup.resizable(False, False)

        # Position near top right
        try:
            x = self.btn_active_count.winfo_rootx() - 100
            y = self.btn_active_count.winfo_rooty() + 35
            popup.geometry(f"+{x}+{y}")
        except Exception:
            pass

        lbl_head = tk.Label(
            popup, text="👥 Network Staff List (Click for Details)", 
            font=("Segoe UI", 9, "bold"), fg=self.AMBER, bg=self.NAVY
        )
        lbl_head.pack(pady=6)

        list_frame = tk.Frame(popup, bg=self.NAVY)
        list_frame.pack(fill="both", expand=True, padx=10, pady=5)

        # Using Treeview instead of Listbox for Color Tag support
        style = ttk.Style()
        style.configure(
            "StaffList.Treeview",
            background="#0f2942",
            foreground="#ffffff",
            fieldbackground="#0f2942",
            rowheight=32,
            font=("Segoe UI", 10, "bold"),
            borderwidth=0
        )
        style.map("StaffList.Treeview", background=[("selected", "#0284c7")])

        tree = ttk.Treeview(list_frame, columns=("StaffName",), show="", style="StaffList.Treeview", selectmode="browse")
        tree.column("StaffName", width=250, anchor="w")
        ...
        # Color Tags: Green for Online, Red for Offline
        tree.tag_configure("ONLINE", foreground="#4ade80", background="#0f2942")
        tree.tag_configure("OFFLINE", foreground="#f87171", background="#0f2942")

        cached_users_data = []

        def load_online_names():
            nonlocal cached_users_data
            try:
                req = urllib.request.Request(WEB_APP_URL, headers={'User-Agent': 'Mozilla/5.0'})
                with urllib.request.urlopen(req, timeout=6) as resp:
                    users = json.loads(resp.read().decode('utf-8'))
                    cached_users_data = users
                    
                    popup.after(0, lambda: [tree.delete(item) for item in tree.get_children()])
                    
                    for idx, u in enumerate(users):
                        is_online = (u.get('status') == 'ONLINE')
                        status_icon = "🟢" if is_online else "🔴"
                        status_text = "ONLINE" if is_online else "OFFLINE"
                        staff_name = u.get('username', 'OFFICIAL_STAFF')
                        
                        display_text = f"  {status_icon}  {staff_name} ({status_text})"
                        tag = "ONLINE" if is_online else "OFFLINE"
                        
                        popup.after(0, lambda t=display_text, tg=tag, i=idx: tree.insert("", "end", iid=str(i), values=(t,), tags=(tg,)))
            except Exception as e:
                popup.after(0, lambda: tree.insert("", "end", values=("  ❌ Failed to load staff list",)))

        threading.Thread(target=load_online_names, daemon=True).start()

        # Step 2: On Clicking Name -> Show Full Details Dialog
        def on_name_select(event):
            selected = tree.selection()
            if selected:
                idx = int(selected[0])
                if idx < len(cached_users_data):
                    selected_user = cached_users_data[idx]
                    popup.destroy()
                    self.show_single_user_detail_dialog(selected_user)

        tree.bind("<<TreeviewSelect>>", on_name_select)

    def show_single_user_detail_dialog(self, user_data):
        """Step 2: Displays full technical & activity details for selected user"""
        dialog = tk.Toplevel(self)
        dialog.title(f"Staff Detail — {user_data.get('username', 'User')}")
        ...
        lbl_title = tk.Label(
            dialog, text=f"👤 Staff Member Profile", 
            font=("Segoe UI", 12, "bold"), fg=self.AMBER, bg=self.NAVY
        )
        lbl_title.pack(pady=12)

        card = tk.Frame(dialog, bg="#0f2942", highlightbackground=self.AMBER, highlightthickness=1)
        card.pack(fill="both", expand=True, padx=20, pady=(0, 20))

        status_str = "🟢 ONLINE" if user_data.get('status') == "ONLINE" else "🔴 OFFLINE"
        
        # Formatted IST Time
        raw_time = user_data.get('last_seen', 'N/A')
        formatted_ist = raw_time
        try:
            ...
            if not pd.isna(dt):
                ...
                formatted_ist = dt.strftime('%d/%m/%Y %I:%M %p')
        except Exception:
            pass

        details = [
            ("Staff Username:", user_data.get('username', 'N/A')),
            ("Computer Name:", user_data.get('computer_name', 'N/A')),
            ("IP Address:", user_data.get('ip_address', 'N/A')),
            ("OS Info:", user_data.get('os_info', 'N/A')),
            ("Live Status:", status_str),
            ("Last Seen (IST):", formatted_ist)
        ]

        for i, (k, v) in enumerate(details):
            row_frame = tk.Frame(card, bg="#0f2942")
            row_frame.pack(fill="x", padx=15, pady=4)

            lbl_k = tk.Label(row_frame, text=k, font=("Segoe UI", 9, "bold"), fg="#94a3b8", bg="#0f2942", width=16, anchor="w")
            lbl_k.pack(side="left")

            fg_c = "#4ade80" if "ONLINE" in v else ("#f87171" if "OFFLINE" in v else "#ffffff")
            lbl_v = tk.Label(row_frame, text=v, font=("Segoe UI", 9, "bold"), fg=fg_c, bg="#0f2942", anchor="w")
